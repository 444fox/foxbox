...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 7 — Dives (bulk-select photos into dive folders)
# ══════════════════════════════════════════════════════════════════════════════

class DivesTab(tk.Frame):
    """
    Thumbnail grid for sorting photos into dives.
    Click to select, Shift+click for a range, then move the selection into
    <folder>/Dives/<dive name>/. Photos already inside Dives/ or rejects/
    are excluded from the grid.
    """
    DIVES_DIR = 'Dives'
    THUMB_W, THUMB_H = 152, 114
    PAD = 6

    def __init__(self, parent):
        super().__init__(parent, bg=BG)
        self._src_var    = tk.StringVar()
        self._dive_var   = tk.StringVar()
        self._status_var = tk.StringVar(value="Pick a folder and click Load.")
        self._photos: list = []      # Paths shown in the grid
        self._widgets: dict = {}     # Path -> thumbnail Label
        self._thumbs: dict = {}      # Path -> PhotoImage (keep refs)
        self._selected: set = set()
        self._last_click = None      # index for shift-click ranges
        self._loading = False
        self._load_gen = 0           # invalidates stale loader threads
        self._build()
    ...
    def _build(self):
        ...
        pf.pack(fill='x', pady=(0,10))
        pf.grid_columnconfigure(1, weight=1)
        path_row(pf, "Photos Folder", self._src_var,
                 lambda: self._browse(), 0)
        tk.Button(pf, text="⟳  LOAD", command=self._load,
                  bg=BLUE, fg=BG, activebackground='#7bbfe8', activeforeground=BG,
                  relief='flat', font=SEMIBOLD, cursor='hand2', padx=14, pady=3
                  ).grid(row=0, column=3, padx=(0,12), pady=6)

        # Status row
        tk.Label(c, textvariable=self._status_var, bg=BG, fg=DIM,
                 font=UI, anchor='w').pack(fill='x', pady=(0,6))

        # Bottom bar first (never clipped)
        bar = tk.Frame(c, bg=BG)
        bar.pack(side='bottom', fill='x', pady=(10,0))

        # Thumbnail grid in a scrollable canvas
        gframe = tk.Frame(c, bg='#111318', highlightthickness=1,
                          highlightbackground=DIM)
        gframe.pack(fill='both', expand=True)
        self._gcanvas = tk.Canvas(gframe, bg='#111318', highlightthickness=0)
        vsb = ttk.Scrollbar(gframe, orient='vertical', command=self._gcanvas.yview)
        self._gcanvas.configure(yscrollcommand=vsb.set)
        vsb.pack(side='right', fill='y')
        self._gcanvas.pack(side='left', fill='both', expand=True)
        self._grid = tk.Frame(self._gcanvas, bg='#111318')
        self._gwin = self._gcanvas.create_window((0,0), window=self._grid, anchor='nw')
        self._grid.bind('<Configure>', lambda e: self._gcanvas.configure(
            scrollregion=self._gcanvas.bbox('all')))
        self._gcanvas.bind('<Configure>', self._on_canvas_resize)
        for w in (self._gcanvas, self._grid):
            w.bind('<MouseWheel>', lambda e: self._gcanvas.yview_scroll(
                -1 if e.delta > 0 else 1, 'units'))

        # Bottom bar contents
        tk.Button(bar, text="Select All", command=self._select_all,
                  bg=PANEL, fg=TEXT, activebackground=BG, relief='flat',
                  font=UI, cursor='hand2', padx=12, pady=6).pack(side='left')
        tk.Button(bar, text="Select None", command=self._select_none,
                  bg=PANEL, fg=DIM, activebackground=BG, relief='flat',
                  font=UI, cursor='hand2', padx=12, pady=6
                  ).pack(side='left', padx=(8,0))
        self._move_btn = tk.Button(bar, text="🌊  MOVE TO DIVE",
                  command=self._move_to_dive,
                  bg=GREEN, fg=BG, activebackground='#6fd09d', activeforeground=BG,
                  relief='flat', font=SEMIBOLD, cursor='hand2', padx=18, pady=6)
        self._move_btn.pack(side='right')
        self._dive_combo = ttk.Combobox(bar, textvariable=self._dive_var,
                                        font=UI, width=28)
        self._dive_combo.pack(side='right', padx=(0,10))
        tk.Label(bar, text="Dive name:", bg=BG, fg=TEXT, font=LABEL
    ...
    def _excluded(self, f: Path, root: Path) -> bool:
        parts = {p.name.lower() for p in f.parents if p != root.parent}
        return 'rejects' in parts or self.DIVES_DIR.lower() in parts

    def _load(self):
        src = self._src_var.get().strip()
        if not src:
            messagebox.showerror("Missing Path", "Please pick a folder.")
            return
        root = Path(src)
        if not root.exists():
            messagebox.showerror("Not Found", f"Folder not found:\n{src}")
        ...
        for w in self._grid.winfo_children():
            w.destroy()
        self._photos = [f for f in scan_media(root)
                        if f.suffix.lower() in PHOTO_EXT
                        and not self._excluded(f, root)]
        self._photos.sort(key=lambda f: str(f).lower())
        self._widgets.clear(); self._thumbs.clear()
        self._selected.clear(); self._last_click = None
        self._update_status()
        # existing dive names for the dropdown
        dives_root = root / self.DIVES_DIR
        names = sorted(d.name for d in dives_root.iterdir() if d.is_dir()) \
                if dives_root.exists() else []
        self._dive_combo.configure(values=names)
        if not self._photos:
            self._status_var.set("No photos found (Dives/ and rejects/ excluded).")
            return
        # thumbnails decode on a worker thread and land in a queue; the main
        # thread polls the queue (worker threads must not touch Tk directly)
        self._loading = True
        self._pending = []
        self._pending_lock = threading.Lock()
        threading.Thread(target=self._thumb_worker, args=(gen,), daemon=True).start()
        self.after(30, lambda: self._poll_thumbs(gen))

    def _thumb_worker(self, gen):
        for i, p in enumerate(self._photos):
            if gen != self._load_gen:
                return
            try:
                with Image.open(p) as img:
                    img.draft('RGB', (self.THUMB_W*2, self.THUMB_H*2))
                    img = ImageOps.exif_transpose(img)
                    img.thumbnail((self.THUMB_W, self.THUMB_H), Image.BILINEAR)
                    pil = img.convert('RGB')
            except Exception:
                pil = Image.new('RGB', (self.THUMB_W, self.THUMB_H), (60,60,60))
            with self._pending_lock:
                self._pending.append((p, pil, i))
        with self._pending_lock:
            self._pending.append(None)   # end marker

    def _poll_thumbs(self, gen):
        if gen != self._load_gen:
            return
        with self._pending_lock:
    ...
    def _cols(self):
        w = self._gcanvas.winfo_width()
        return max(1, w // (self.THUMB_W + 2*self.PAD + 8))

    def _add_thumb(self, p, pil, i, gen):
        if gen != self._load_gen:
            return
        ph = ImageTk.PhotoImage(pil)
        self._thumbs[p] = ph
        l = tk.Label(self._grid, image=ph, bg='#111318', bd=0,
                     highlightthickness=3, highlightbackground='#111318',
                     cursor='hand2')
        cols = self._cols()
        l.grid(row=i // cols, column=i % cols, padx=self.PAD, pady=self.PAD)
        l.bind('<Button-1>', lambda e, p=p: self._clicked(p, e))
        l.bind('<MouseWheel>', lambda e: self._gcanvas.yview_scroll(
            -1 if e.delta > 0 else 1, 'units'))
        self._widgets[p] = l
        if i % 25 == 0:
            self._update_status(loading_at=i+1)

    def _on_canvas_resize(self, event):
        self._gcanvas.itemconfigure(self._gwin, width=event.width)
        self._reflow()
    ...
    def _reflow(self):
        ...
        for p in self._photos:
            l = self._widgets.get(p)
            if l is not None:
                l.grid_configure(row=i // cols, column=i % cols)
                i += 1

    # ── Selection ─────────────────────────────────────────────────────────────

    def _set_selected(self, p, on):
        if p not in self._widgets:
            return
        if on:
            self._selected.add(p)
            self._widgets[p].configure(highlightbackground=BLUE)
        else:
            self._selected.discard(p)
            self._widgets[p].configure(highlightbackground='#111318')

    def _clicked(self, p, event):
        idx = self._photos.index(p)
        if (event.state & 0x0001) and self._last_click is not None:   # Shift
            lo, hi = sorted((idx, self._last_click))
            for q in self._photos[lo:hi+1]:
                self._set_selected(q, True)
        else:
            self._set_selected(p, p not in self._selected)
        self._last_click = idx
        self._update_status()

    def _select_all(self):
        for p in self._photos:
            self._set_selected(p, True)
        self._update_status()

    def _select_none(self):
        for p in list(self._selected):
            self._set_selected(p, False)
        self._update_status()

    def _update_status(self, loading_at=None):
        n = len(self._photos)
        msg = f"{len(self._selected)} of {n} selected"
        if self._loading and loading_at:
            msg += f"   (loading thumbnails {loading_at}/{n}…)"
        msg += "   —   click = select, Shift+click = range"
        self._status_var.set(msg)

    # ── Move to dive ──────────────────────────────────────────────────────────

    def _move_to_dive(self):
        name = self._dive_var.get().strip()
        if not self._selected:
            messagebox.showinfo("Nothing Selected", "Select some photos first.")
            return
        if not name:
            messagebox.showerror("No Dive Name",
                                 "Type a dive name (or pick one from the dropdown).")
            return
        safe = ''.join(ch for ch in name if ch not in '\\/:*?"<>|').strip()
        if not safe:
            messagebox.showerror("Bad Name", "That name has no usable characters.")
            return
        root = Path(self._src_var.get().strip())
        dest = root / self.DIVES_DIR / safe
        dest.mkdir(parents=True, exist_ok=True)
        moved = errors = 0
        for p in sorted(self._selected, key=lambda f: str(f).lower()):
            target = dest / p.name
            n2 = 1
            while target.exists():
                target = dest / f"{p.stem}_{n2}{p.suffix}"
                n2 += 1
            try:
                shutil.move(str(p), str(target))
                moved += 1
                w = self._widgets.pop(p, None)
                if w: w.destroy()
                self._thumbs.pop(p, None)
        ...
        self._reflow()
        # refresh dive dropdown with the (possibly new) name
        names = list(self._dive_combo.cget('values'))
        if safe not in names:
            self._dive_combo.configure(values=sorted(names + [safe]))
        self._status_var.set(
            f"Moved {moved} photo(s) to Dives/{safe}"
            + (f" — {errors} error(s)" if errors else "")
            + f".   {len(self._photos)} photo(s) left.")
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        nb.add(self._dives_tab,  text="  🌊  Dives  ")
